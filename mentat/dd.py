...
def run():  # Define the run function for the page
    ...
    if uploaded_file is not None:
        try:
            ...
            st.dataframe(df.head(100))  # Display the first 100 rows of the CSV
            
            # Convert the first 100 rows to a string for appending to the prompt
    ...
    if df is not None:
        ...
        if dedupe_fields:
            ...
            st.markdown("### Data after Deduplication (Showing First 100 Rows)")
            st.dataframe(deduplicated_df.head(100))  # Show first 100 rows after deduplication

            # Convert the first 100 rows of the deduplicated dataframe to string for the prompt
            deduplicated_csv_content = deduplicated_df.head(100).to_string(index=False)
        else:
    ...
    if st.button("Submit"):
        if prompt.strip() or (include_csv and csv_content):  # Validate input
